...
// Read OmniVoice special tokens from KV omnivoice.special.{denoise, lang_*,
...
static bool bpe_load_omnivoice_specials(BPETokenizer * tok, const char * gguf_path) {
    struct gguf_init_params gp  = { true, NULL };
    struct gguf_context *   ctx = gguf_init_from_file(gguf_path, gp);
    if (!ctx) {
    ...
    static const char * keys[] = {
        "omnivoice.special.denoise",        "omnivoice.special.lang_start",   "omnivoice.special.lang_end",
        "omnivoice.special.instruct_start", "omnivoice.special.instruct_end", "omnivoice.special.text_start",
    ...
    gguf_free(ctx);
    fprintf(stderr, "[BPE] Registered %d OmniVoice special tokens (total specials=%zu)\n", n_added,
            tok->specials.size());
    return true;
}
